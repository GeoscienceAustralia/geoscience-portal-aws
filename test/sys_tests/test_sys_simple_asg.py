...
runcmd:
 - service httpd start
    ...
    subnet = template.add_resource(ec2.Subnet('MySubnet',
                                   AvailabilityZone='ap-southeast-2a',
                                   VpcId=Ref(vpc),
                                   CidrBlock='10.0.1.0/24'))

    internet_gateway = template.add_resource(ec2.InternetGateway('MyInternetGateway'))
    internet_gateway_attachment = template.add_resource(ec2.VPCGatewayAttachment('MyInternetGatewayAttachment',
                                                                                 VpcId=Ref(vpc),
