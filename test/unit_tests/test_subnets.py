...
def test_pub_or_pri():
    ...
    pub_route_table = stack.add_resource(ec2.RouteTable('MyUnitPublicRouteTable',
                                                        VpcId=Ref(stack.vpc)))
    pri_route_table = stack.add_resource(ec2.RouteTable('MyUnitPrivateRouteTable',
                                                        VpcId=Ref(stack.vpc)))
    for a in az:
        helper_pub_subnet = create_subnet(stack=stack, az=a, route_table=pub_route_table)
        assert_equals(helper_pub_subnet.pub_or_pri, 'Public')
        helper_pri_subnet = create_subnet(stack=stack, az=a, route_table=pri_route_table)
