...
def main(args):
    ...
    add_security_group_ingress(template, web_sg, "tcp", HTTP_PORT_TC, HTTP_PORT_TC, cidr=PUBLIC_CIDR)
    ...
    web_asg = add_auto_scaling_group(template, 1, [template.public_subnet], launch_configuration=web_launch_config, dependson=[template.internet_gateway.title], app_name=appname)
